...
export default function ProductGrid() {
  const { productCollection_id, productCollection_name } = useZustand();
  const [data, setData] = useState<Data[]>([]);
  const [like, setlike] = useState(false);
  useEffect(() => {
    axios
      .get(
        `http://127.0.0.1:8000/api/features_product/collection/${productCollection_id}?limit=6`
      )
